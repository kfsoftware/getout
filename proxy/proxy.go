...
func New(lconn net.Conn, rconn net.Conn) *Proxy {
	return &Proxy{
		lconn: lconn,
		//laddr:  laddr,
		//raddr:  raddr,
...
// locally
func NewTLSUnwrapped(lconn, rconn *net.TCPConn, laddr, raddr *net.TCPAddr, addr string) *Proxy {
	p := New(lconn, rconn)
	p.tlsUnwrapp = true
	p.tlsAddress = addr
	return p
...
func (p *Proxy) Start() {
	...
	//display both ends
	//log.Infof("Opened %s >>> %s", p.lconn.String(), p.raddr.String())
	...
	//wait for close...
	<-p.errsig
	log.Infof("Closed (%d bytes sent, %d bytes recieved)", p.sentBytes, p.receivedBytes)
}
